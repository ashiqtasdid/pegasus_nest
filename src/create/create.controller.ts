...
`;
  ...
  // New API chat endpoints to match frontend expectations
  @Post('api/chat/send')
  async apiChatSend(
    @Body() chatData: ChatRequestDto,
  ): Promise<{ success: boolean; response?: string; error?: string }> {
    try {
      ...
      console.log(
        `API Chat request received for plugin: ${chatData.pluginName}, message: ${chatData.message}`,
      );
